...
public class Zipline : MonoBehaviour
{
    ...
    public void AttachPlayer(int playerId, Transform playerTransform)
    {
        if (playerId == 1 && !player1Interacted)
        {
            player1Interacted = true;
            // 플레이어 1을 지정된 위치로 이동
            AttachPlayerToPosition(playerTransform, player1AttachPoint.position);
        }
        else if (playerId == 2 && !player2Interacted)
        {
            player2Interacted = true;
            // 플레이어 2를 지정된 위치로 이동
            AttachPlayerToPosition(playerTransform, player2AttachPoint.position);
        }
    ...
    private void AttachPlayerToPosition(Transform playerTransform, Vector3 attachPosition)
    {
